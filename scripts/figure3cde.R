...
# ggsave("stats/hist_aFC.png", width = 3, height = 2.5)
...
p2 <- eqtls |>
    ...
    # stat_bin(geom = "step", bins = 38, boundary = 0.5,
    #          position = position_nudge(x = -0.5 * (0.5 / 38))) +
    # geom_histogram(data = all_snps, bins = 38, boundary = 0.5, color = "red", fill = NA) +
    # geom_histogram(data = all_snps, bins = 38, boundary = 0.5, fill = "red", alpha = 0.3) +
    # geom_density(data = all_snps, adjust = 5, size = 0.8, outline.type = "full", show.legend = FALSE) +
...
# ggsave("stats/hist_MAF.png", width = 3, height = 2.5)
...
# ggsave("stats/hist_TSS.png", width = 3, height = 2.5)
