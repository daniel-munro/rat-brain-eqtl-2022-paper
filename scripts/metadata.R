...
reads <- read_tsv(
    "data/qc/metadata/read_stats.txt",
    col_types = cols(
        file = "c",
...
# file_locations
d <- d |>
    left_join(fastq, by = "library", relationship = "one-to-one")

# raw_reads, uniq_mapped_reads, uniq_mapped_ratio, unmapped_reads_ratio
d <- d |>
    left_join(reads, by = "library", relationship = "one-to-one")
...
d <- d |>
    ...
    mutate(
        geno_match_comment = if_else(
            library %in% names(geno_dups),
